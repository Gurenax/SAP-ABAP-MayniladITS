***************************************************************************************************
* PROGRAM ID      : ZPRG_GENERATE_SOA_V2
...
* 12/03/2013 | GDIMALIWAT | Added conditions and sub-routines for Archiving     | TN#45188
*--------------------------------------------------------------------------------------------------

REPORT  zprg_generate_soa_v2 LINE-SIZE 1000 NO STANDARD PAGE HEADING.

*&---------------------------------------------------------------------*
*& Includes - (<include name/s>)
*&
INCLUDE: zinclude_esoa, lzfg_data_entityf01.
...
*& Types
*&
TYPES: BEGIN OF ty_dberchz,
          belnr    TYPE e_belnr,
          belzeile TYPE belzeile,
          belzart  TYPE belzart,
          nettobtr TYPE nettobtr,
       END OF ty_dberchz.
...
*& Data
*&
DATA: gv_archived TYPE c LENGTH 1.
DATA: gt_dberchz  TYPE TABLE OF ty_dberchz.
...
SELECT-OPTIONS:
 s_can    FOR fkkvkp-vkont OBLIGATORY NO-EXTENSION NO INTERVALS DEFAULT '50523245'. "-- CAN
...
START-OF-SELECTION.
  ...
  IF sy_subrc EQ 0.
    ...
    IF sy-subrc NE 0.
      MESSAGE s398(00) WITH 'Contract Account not found.' DISPLAY LIKE 'E'.
      EXIT.
*      LEAVE PROGRAM.
    ENDIF.
    ...
    LOOP AT it_soa ASSIGNING <fs_soa>.
      ...
      IF sy_subrc EQ 0.
        ...
        DATA: lv_begabrpe TYPE begabrpe.

        "-- Reset archived tag
        CLEAR gv_archived.

        SELECT SINGLE belnr begabrpe
          INTO (<fs_soa>-belnr, lv_begabrpe)
        FROM   erch
        ...
        IF sy-subrc NE 0.
          ...
          "-- Extract billing document from archive
          PERFORM get_archived_bill USING <fs_soa>-vkont <fs_soa>-adatsoll
                                    CHANGING <fs_soa>-belnr lv_begabrpe.
        ENDIF.

        PERFORM get_previous_mtr_reading_date USING    lv_begabrpe
                                              CHANGING wa_zbilldet-prevrdgdt.

        "-- Added by GDIMALIWAT 12/03/2013 - ARCHIVING
        IF gv_archived NE 'X'.
          PERFORM get_consumption USING    <fs_soa>-belnr
                                           <fs_soa>-adatsoll
                                           <fs_soa>-anlage
                                  CHANGING wa_zbilldet-consump.
        ELSE.
          PERFORM get_consumption_archived USING    <fs_soa>-belnr
                                                    <fs_soa>-adatsoll
                                                    <fs_soa>-anlage
                                           CHANGING wa_zbilldet-consump.
        ENDIF.
...
*                                          sy_subrc.
        ...
        mc_conversion_alpha <fs_soa>-belnr.

*        DATA: lv_tar_reduction TYPE nettobtr.

        "-- Added by GDIMALIWAT 12/03/2013 - ARCHIVING
        IF gv_archived NE 'X'.
          PERFORM get_billing_details USING    <fs_soa>-belnr
                                      CHANGING <fs_soa>-basic
                                               <fs_soa>-fcda
                                               <fs_soa>-env
                                               <fs_soa>-sew
                                               <fs_soa>-msc
...
*                                               lv_tar_reduction
                                               ...
                                               <fs_soa>-pia.
        ELSE.
          PERFORM get_billing_details_archived USING    <fs_soa>-belnr
                                               CHANGING <fs_soa>-basic
                                                        <fs_soa>-fcda
                                                        <fs_soa>-env
                                                        <fs_soa>-sew
...
*        IF sy_subrc NE 0.
        IF <fs_soa>-basic IS INITIAL.
          MESSAGE s398(00) WITH 'No Billing Summary Records' DISPLAY LIKE 'E'. EXIT.
        ENDIF.
...
*        ENDIF.
        ...
        PERFORM get_charges_soa USING    <fs_soa>-vkont
...
* Append to paydoc all details for MISC **
  ...
  LOOP AT it_paydoc ASSIGNING <fs_paydoc>.
    ...
    PERFORM determine_tax_code_soa USING    f_can
                                            f_instal_no
                                            txttag
...
FORM disctag.
  ...
  IF sy-subrc EQ 0.
    ...
    PERFORM get_pua_from_download_soa USING    <fs_soa>-vkont
                                               <fs_soa>-adatsoll
                                      CHANGING <fs_soa>-pua.
  ...
  ELSE.
    ...
    PERFORM get_pua_from_fm_soa USING    <fs_soa>-vkont
                                         lv_budat
                                         wa_zbilldet-opbel
                                CHANGING <fs_soa>-pua
                                         lv_due_date.
...
FORM display_details .
  ...
  LOOP AT it_soa ASSIGNING <fs_soa>.
    ...
    m_append_bill_sum:
         ...
         <fs_soa>-stm           0       abap_true   'Special Transitory Mechanism',
...
*       text
*----------------------------------------------------------------------*
FORM get_charges_soa  USING    pv_vkont         TYPE vkont_kk
  ...
  FIELD-SYMBOLS:
        ...
        <fs_fkkop>  TYPE fkkop,
        <fs_clear_docs> TYPE ty_clear_docs.
...
*  AND    budat BETWEEN <fs_soa>-prevrdgdt AND <fs_soa>-presrdgdt
  ...
  LOOP AT it_dfkkop ASSIGNING <fs_dfkkop>.
    ...
    IF <fs_dfkkop>-augbl IS NOT INITIAL.
      ...
      LOOP AT lt_fkkop ASSIGNING <fs_fkkop> WHERE faedn GE <fs_soa>-prevrdgdt
                                              AND faedn LE <fs_soa>-presrdgdt
        ...
        LOOP AT lt_sfkkop ASSIGNING <fs_sfkkop> WHERE faedn GE lv_start_bill "<fs_soa>-prevrdgdt  "tmp_firstdayofmonth
          ...
          IF <fs_sfkkop>-faedn LE sy-datum.
            IF <fs_sfkkop>-augbl IS NOT INITIAL AND <fs_sfkkop>-augbl NE '*'.
              ...
              "-- Get Installment Plan Due
              PERFORM get_instal_due USING    <fs_sfkkop>-augbl
                                              <fs_sfkkop>-betrh
                                              lv_start_bill
                                              lv_end_bill
                                              <fs_soa>-reading_date
                                     CHANGING pv_instal_due.

            ELSEIF <fs_sfkkop>-augbl IS INITIAL.
              CLEAR: pv_instal_due.

              "-- Deduct Cleared Amount
              <fs_sfkkop>-betrh = <fs_sfkkop>-betrh - <fs_sfkkop>-augbt.
              pv_instal_due     = <fs_sfkkop>-betrh.
            ELSEIF <fs_sfkkop>-augbl EQ '*'.
              "-- Get List of Clearing Documents
              SELECT augbl
                INTO TABLE lt_clear_docs
              FROM   dfkkop
              WHERE  opbel EQ <fs_sfkkop>-opbel
              AND    opupw EQ <fs_sfkkop>-opupw
              AND    opupk EQ <fs_sfkkop>-opupk
              AND    bukrs EQ <fs_sfkkop>-bukrs
              AND    augst EQ <fs_sfkkop>-augst
              AND    gpart EQ <fs_sfkkop>-gpart
              AND    vkont EQ <fs_sfkkop>-vkont
              AND    hvorg EQ <fs_sfkkop>-hvorg
              AND    tvorg EQ <fs_sfkkop>-tvorg.

              IF sy-subrc EQ 0.
                LOOP AT lt_clear_docs ASSIGNING <fs_clear_docs>.
                  "-- Get Installment Plan Due
                  PERFORM get_instal_due USING    <fs_clear_docs>-augbl
                                                  <fs_sfkkop>-betrh
                                                  lv_start_bill
                                                  lv_end_bill
                                                  <fs_soa>-reading_date
...
*
*----------------------------------------------------------------------*
FORM get_pua_from_download_soa  USING    pv_vkont        LIKE fkkvkp-vkont
...
FORM get_bill_reference_number  USING    pv_vkont       TYPE vkont_kk
  ...
  IF pv_lowest_date GT lv_next_mr_date.
    ...
    ELSE.
      "-- Get from archive - GDIMALIWAT 12/05/2013
      PERFORM get_refno_duedate_archive TABLES lt_refno
                                         USING pv_vkont
                                               lv_next_mr_date
                                               pv_lowest_date.
    ENDIF.
...
*----------------------------------------------------------------------*
FORM get_pua_from_fm_soa  USING    pv_vkont    TYPE vkont_kk
...
FORM get_reading_details  USING    pv_vkont        TYPE vkont_kk
                          ...
                          CHANGING pv_subrc        TYPE sysubrc
                                   ...
                                   pv_pres_mr      TYPE zbilldet-presrdg
                                   pv_pres_mr_date TYPE zbilldet-presrdgdt.
    ...
    IF lt_eabl[] IS NOT INITIAL.
      ...
      IF sy-subrc EQ 0.
        ...
        IF sy-subrc EQ 0.
          ...
          pv_pres_mr      = ceil( <fs_eabl>-readingresult ).
          pv_pres_mr_date = <fs_eabl>-actualmrdate.
        ENDIF.
      ENDIF.
    ENDIF.
  ENDIF.
...
*----------------------------------------------------------------------*
FORM determine_tax_code_soa  USING    pv_can       TYPE vkont_kk
...
FORM get_previous_consumptions  USING    pv_can               TYPE vkont_kk
                                ...
                                CHANGING pv_prev_months       TYPE char80  "char45 TN#30334 Changed by Ann 20130325
                                         ...
                                         pv_previous_reading  TYPE zbilldet-prevrdg.
  ...
  DATA: lv_belnr       TYPE e_belnr,
        lv_mr_sched    TYPE adatsoll.
...
*    AND    adatsoll BETWEEN pv_first_day AND lv_last_day.
    ...
    IF gv_archived ne 'X'.
      "-- Get Document Number
      SELECT SINGLE belnr adatsoll
        INTO (lv_belnr, lv_mr_sched)
      FROM   erch
      WHERE  vkont     EQ pv_can
      AND    adatsoll  BETWEEN pv_first_day AND lv_last_day
      AND    stornodat EQ ``.
    ELSE.
      "-- Get Document Number from archive
      PERFORM GET_ARCHIVED_BILL_DATE_RANGE USING pv_can
                                                 pv_first_day
                                                 lv_last_day
                                        CHANGING lv_belnr
                                                 lv_mr_sched
    ...
    IF sy-subrc EQ 0.
      "-- Get Consumption of Previous Month
      IF gv_archived ne 'X'.
        PERFORM get_consumption USING    lv_belnr
                                         lv_mr_sched
                                         pv_installation
                                CHANGING lv_consumption2.
      ELSE.
        "-- Get consumption from archive
        PERFORM get_consumption_archive USING    lv_belnr
                                                 lv_mr_sched
                                                 pv_installation
                                        CHANGING lv_consumption2.
      ENDIF.
...
*      IF sy-subrc EQ 0.
      IF lv_consumption2 IS NOT INITIAL.
        WRITE: lv_consumption2 TO lv_consumption ROUND 0 DECIMALS 0.

        DATA: lv_string TYPE string.

        lv_string = lv_consumption2.
      ...
      IF sy-index EQ 1.
        PERFORM get_meter_reading USING     pv_installation
                                            lv_mr_sched
                                  CHANGING  pv_previous_reading.
      ENDIF.
    ELSE.
      lv_consumption = 0.
    ENDIF.
...
**       Get Installment Plan Due
**----------------------------------------------------------------------*
*FORM get_instal_due  USING    pv_augbl         TYPE augbl_kk
*                              pv_betrh         TYPE betrh_kk
*                              pv_start_bill    TYPE sydatum
*                              pv_end_bill      TYPE sydatum
*                              pv_reading_date  TYPE adattats
*                     CHANGING pv_instal_due    TYPE betrw_kk.
*
...
*      t_fkkop = lt_fkkop.
*
*  pv_instal_due = pv_betrh.
*
*  LOOP AT lt_fkkop ASSIGNING <fs_fkkop> WHERE faedn GE pv_start_bill
*                                         AND  faedn LE pv_end_bill
*                                         AND  hvorg EQ '0080'.
*
*    IF pv_reading_date GE <fs_fkkop>-augdt.
*      pv_instal_due = pv_instal_due - <fs_fkkop>-augbt.
*    ENDIF.
*  ENDLOOP.
*ENDFORM.                    " GET_INSTAL_DUE
...
*       i_maxrows                 = 1
  ...
  IF sy-subrc EQ 0.
    ...
    IF sy-subrc EQ 0.
      ...
      pv_begabrpe = lv_erch-begabrpe.
    ENDIF.
  ENDIF.
ENDFORM.                    " GET_ARCHIVED_BILL

*&---------------------------------------------------------------------*
*&      Form  GET_ARCHIVED_BILL_DATE_RANGE
*&---------------------------------------------------------------------*
*       Extract bill document from archive by date range (ADATSOLL)
*----------------------------------------------------------------------*
*      -->PV_VKONT          Contract Account Number
*      -->PV_ADATSOLL_FROM  Scheduled Meter Reading Date
*      -->PV_ADATSOLL_TO    Scheduled Meter Reading Date
*      <--PV_BELNR          Billing Document Number
*      <--PV_BEGABRPE       Start of billing period
*----------------------------------------------------------------------*
FORM get_archived_bill_date_range  USING    pv_vkont           type erch-vkont
                                            pv_adatsoll_from   type erch-adatsoll
                                            pv_adatsoll_to     type erch-adatsoll
                                   CHANGING pv_belnr           type erch-belnr
                                            pv_adatsoll        type erch-adatsoll
                                            pv_subrc           type sysubrc.
  ...
  DATA:   lt_selections TYPE rsds_frange_t,
          lt_vkont      TYPE TABLE OF rsdsselopt,
          lt_adatsoll   TYPE TABLE OF rsdsselopt,
          lt_stornodat  TYPE TABLE OF rsdsselopt.

  FIELD-SYMBOLS: <fs_doc_data>   TYPE STANDARD TABLE,
                 <fs_vkont>      LIKE LINE OF lt_vkont,
                 <fs_selections> LIKE LINE OF lt_selections,
                 <fs_selopt>     TYPE rsdsselopt,
                 <fs_any>        TYPE ANY.

  REFRESH lt_selections.

  "-- Input value for VKONT
  APPEND INITIAL LINE TO lt_vkont ASSIGNING <fs_selopt>.
  <fs_selopt>-sign   = 'I'.
  <fs_selopt>-option = 'EQ'.
  <fs_selopt>-low    = pv_vkont.

  "-- Input value for ADATSOLL
  APPEND INITIAL LINE TO lt_adatsoll ASSIGNING <fs_selopt>.
  <fs_selopt>-sign   = 'I'.
  <fs_selopt>-option = 'BT'.
  <fs_selopt>-low    = pv_adatsoll_from.
  <fs_selopt>-high   = pv_adatsoll_to.

  "-- Input value for STORNODAT
  APPEND INITIAL LINE TO lt_vkont ASSIGNING <fs_selopt>.
  <fs_selopt>-sign   = 'I'.
  <fs_selopt>-option = 'EQ'.
  <fs_selopt>-low    = '00000000'.

  APPEND INITIAL LINE TO lt_selections ASSIGNING <fs_selections>.
  <fs_selections>-fieldname = 'VKONT'.
  LOOP AT lt_vkont ASSIGNING <fs_any>.
    APPEND INITIAL LINE TO <fs_selections>-selopt_t ASSIGNING <fs_selopt>.
    MOVE-CORRESPONDING <fs_any> to <fs_selopt>.
  ENDLOOP.

  APPEND INITIAL LINE TO lt_selections ASSIGNING <fs_selections>.
  <fs_selections>-fieldname = 'ADATSOLL'.
  LOOP AT lt_adatsoll ASSIGNING <fs_any>.
    APPEND INITIAL LINE TO <fs_selections>-selopt_t ASSIGNING <fs_selopt>.
    MOVE-CORRESPONDING <fs_any> to <fs_selopt>.
  ENDLOOP.

  APPEND INITIAL LINE TO lt_selections ASSIGNING <fs_selections>.
  <fs_selections>-fieldname = 'STORNODAT'.
  LOOP AT lt_stornodat ASSIGNING <fs_any>.
    APPEND INITIAL LINE TO <fs_selections>-selopt_t ASSIGNING <fs_selopt>.
    MOVE-CORRESPONDING <fs_any> to <fs_selopt>.
  ENDLOOP.
*  lv_selection-vkont     = pv_vkont.
...
*       i_maxrows                 = 1
  ...
  IF sy-subrc EQ 0.
    ...
    IF sy-subrc EQ 0.
      ...
      READ TABLE lt_erch INDEX 1 INTO lv_erch.
      pv_belnr    = lv_erch-belnr.
      pv_adatsoll = lv_erch-adatsoll.
      pv_subrc    = sy-subrc.
    ENDIF.
  ENDIF.
ENDFORM.                    " GET_ARCHIVED_BILL_DATE_RANGE
...
*&---------------------------------------------------------------------*
*&      Form  GET_CONSUMPTION_ARCHIVED
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*      -->IV_BILLING_PERIOD  text
*      <--CV_CONSUMPTION1    text
*----------------------------------------------------------------------*
FORM get_consumption_archived  USING    pv_belnr        TYPE erch-belnr
                                        pv_adatsoll     TYPE eablg-adatsoll
                                        pv_installation TYPE eablg-anlage
                               CHANGING pv_consumption  TYPE char15.

  DATA: lv_amount     TYPE v_abrmenge,
        lv_cera       TYPE nettobtr,
        lv_tmp_amount TYPE string,
        lv_temp       TYPE p DECIMALS 0.

  DATA: lv_ablbelnr   TYPE ablbelnr,
        lv_istablart  TYPE istablart.

  CONSTANTS: co_adj TYPE char3 VALUE 'ADJ',
             co_ave TYPE char3 VALUE 'AVE'.

*  SELECT SUM( v_abrmenge )
*    INTO lv_amount
*    FROM dberchz1
*    WHERE belnr = pv_belnr
*    AND ( ( belzart = 'IQUANT' AND ein01 = 'WQUANT___1' ) OR (  ein01 LIKE 'WQUANT%' AND
*    ( tarifnr = 'RESAWS' OR tarifnr = 'COMAWS' OR tarifnr = 'COMAWW' OR tarifnr = 'COMAW'
*    OR tarifnr = 'RESAWW' OR tarifnr = 'RESAW' OR tarifnr = 'RESAS' OR tarifnr = 'COMAS' ) ) )
*    AND v_abrmenge GE '0'.

  PERFORM get_archived_consumption    USING pv_belnr
                                   CHANGING lv_amount.

  pv_consumption = lv_amount.
  CONDENSE pv_consumption.
...
*    WHERE dberchz3~belnr  = pv_belnr
*      AND dberchz3~mwskz <> space
*      AND dberchz3~preistuf = space
*      AND preis LIKE 'CERA%'.

  PERFORM get_archived_cera  USING pv_belnr
                          CHANGING lv_cera.

  IF ( lv_cera < 0 ).
    lv_cera = lv_cera * -1.
    lv_tmp_amount = lv_cera.
    CONDENSE lv_tmp_amount.
    CONCATENATE '-' lv_tmp_amount INTO lv_tmp_amount.
    lv_cera = lv_tmp_amount.
  ENDIF.

  IF lv_cera > 0.
    " Remove the decimal points( decimals is always zero )
    lv_temp = lv_cera.
    pv_consumption = lv_temp.
  ENDIF.

  SELECT SINGLE ablbelnr
    FROM  eablg
    INTO  lv_ablbelnr
    WHERE anlage   = pv_installation AND
          adatsoll = pv_adatsoll.
  IF sy-subrc = 0.
    SELECT SINGLE istablart
      FROM dberchv
      INTO lv_istablart
      WHERE belnr = pv_belnr AND
            ablbelnr = lv_ablbelnr.
  ENDIF.

  CONDENSE pv_consumption.
  IF NOT pv_consumption IS INITIAL.
    CASE lv_istablart.
      WHEN 93.
        CONCATENATE pv_consumption
                    co_adj
                    INTO pv_consumption
                    SEPARATED BY space.
      WHEN 91.
...
ENDFORM.                    " GET_CONSUMPTION_ARCHIVED

*&---------------------------------------------------------------------*
*&      Form  GET_ARCHIVED_CONSUMPTION
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*      -->PV_BELNR   Billing document number
*      <--PV_AMOUNT  Consumption Amount
*----------------------------------------------------------------------*
FORM get_archived_consumption  USING    pv_belnr     TYPE dberchz1-belnr
                               CHANGING pv_amount    TYPE dberchz1-v_abrmenge.
  TYPES : BEGIN OF ty_selection,
            belnr   TYPE dberchz1-belnr,
          END OF ty_selection.
...
*       i_maxrows                 = 1
  ...
  IF sy-subrc EQ 0.
    ...
    IF sy-subrc EQ 0.
      ...
      CLEAR pv_amount.
      LOOP AT lt_dberchz1 ASSIGNING <fs_dberchz1> WHERE ( ( belzart = 'IQUANT' AND ein01 = 'WQUANT___1' ) OR (  ein01 CP 'WQUANT*' AND
                                                        ( tarifnr = 'RESAWS' OR tarifnr = 'COMAWS' OR tarifnr = 'COMAWW' OR tarifnr = 'COMAW'
                                                        OR tarifnr = 'RESAWW' OR tarifnr = 'RESAW' OR tarifnr = 'RESAS' OR tarifnr = 'COMAS' ) ) )
                                                        AND v_abrmenge GE '0'.
        ADD <fs_dberchz1>-v_abrmenge TO pv_amount.
      ENDLOOP.
      UNASSIGN <fs_dberchz1>.
    ENDIF.
  ENDIF.
ENDFORM.                    " GET_ARCHIVED_CONSUMPTION

*&---------------------------------------------------------------------*
*&      Form  GET_ARCHIVED_CERA
*&---------------------------------------------------------------------*
*       Extract nettobtr from archive
*----------------------------------------------------------------------*
*      -->PV_BELNR      Billing document number
*      <--PV_CERA       CERA amount
*----------------------------------------------------------------------*
FORM get_archived_cera  USING    pv_belnr     TYPE dberchz3-belnr
                        CHANGING pv_cera      TYPE dberchz3-nettobtr.
  TYPES : BEGIN OF ty_selection,
            belnr    TYPE dberchz3-belnr,
          END OF ty_selection.
  ...
  IF sy-subrc EQ 0.
    ...
    IF sy-subrc EQ 0.
      ...
      IF sy-subrc EQ 0.
        ...
        CLEAR pv_cera.
        LOOP AT lt_dberchz1 ASSIGNING <fs_dberchz1>.
          LOOP AT lt_dberchz3 ASSIGNING <fs_dberchz3> WHERE belzeile EQ <fs_dberchz1>-belzeile
                                                        AND mwskz <> space
                                                        AND preistuf = space
                                                        AND preis CP 'CERA*'.
            ADD <fs_dberchz3>-nettobtr TO pv_cera.
          ENDLOOP.
        ENDLOOP.
        UNASSIGN <fs_dberchz1>.
        UNASSIGN <fs_dberchz3>.
      ENDIF.
    ENDIF.
  ENDIF.
ENDFORM.                    " GET_ARCHIVED_CERA

*&---------------------------------------------------------------------*
*&      Form  GET_BILLING_DETAILS_ARCHIVED
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*      -->P_LV_BELNR  text
*----------------------------------------------------------------------*
FORM get_billing_details_archived  USING    iv_belnr
                                   CHANGING cv_basic_charge
                                            cv_fcda
                                            cv_environmental_charge
                                            cv_sewer_charge
                                            cv_maintenance_service_charge
                                            cv_senior_citizen_dscnt
                                            cv_tariff_reduction
                                            cv_transitory_mechanism
                                            cv_payment_incentive_adjustmnt.

  CONSTANTS: co_zbasic TYPE belzart VALUE 'ZBASIC', " Basic Charge
             co_zfcda  TYPE belzart VALUE 'ZFCDA',  " Foreign Currency Differential Adjustment(FCDA)
             co_zenv   TYPE belzart VALUE 'ZENV',   " Environmental Charge
             co_zsewer TYPE belzart VALUE 'ZSEWER', " Sewer Charge
             co_zmsc   TYPE belzart VALUE 'ZMSC',   " Maintenance Service Charge (MSC)
             co_zdissc TYPE belzart VALUE 'ZDISSC', " Senior Citizen Discount
             co_zotr   TYPE belzart VALUE 'ZOTR',   " 2012 One Time Tariff Reduction
             co_zstm   TYPE belzart VALUE 'ZSTM',   " Special Transitory Mechanism
             co_zpia   TYPE belzart VALUE 'ZPIA',   " Payment Incentive Adjustment
             co_cera   TYPE belzart VALUE 'ZCERA',  " MWSI Currency exchange Rate Ad
             co_dispa  TYPE belzart VALUE 'ZDISPA', " PA discount
             co_discn  TYPE belzart VALUE 'ZDISCN', " MWSI discounted consumptions
             co_pa     TYPE belzart VALUE 'ZPA'.    " Pre payment adjustment

  FIELD-SYMBOLS: <fs_dberchz> TYPE ty_dberchz.

  CLEAR: cv_basic_charge,
         cv_fcda,
         cv_environmental_charge,
         cv_sewer_charge,
         cv_maintenance_service_charge,
...
*    FROM dberchz1 AS a INNER JOIN dberchz3 AS b
*      ON  a~belnr      = b~belnr
*      AND a~belzeile   = b~belzeile
*    WHERE a~belnr      = iv_belnr.

  REFRESH gt_dberchz.
  CLEAR gt_dberchz.
  PERFORM get_archived_charges TABLES gt_dberchz
                                USING iv_belnr.

  IF sy-subrc = 0.

    LOOP AT gt_dberchz ASSIGNING <fs_dberchz>.

      CASE <fs_dberchz>-belzart.
        WHEN co_zbasic OR co_cera OR co_dispa OR co_discn OR co_pa. " Basic Charge or MWSI Currency exchange Rate Ad

          cv_basic_charge = cv_basic_charge + <fs_dberchz>-nettobtr.

        WHEN co_zfcda.  " Foreign Currency Differential Adjustment(FCDA)

          cv_fcda = cv_fcda + <fs_dberchz>-nettobtr.

        WHEN co_zenv.   " Environmental Charge

          cv_environmental_charge = cv_environmental_charge + <fs_dberchz>-nettobtr.

        WHEN co_zsewer. " Sewer Charge

          cv_sewer_charge = cv_sewer_charge + <fs_dberchz>-nettobtr.

        WHEN co_zmsc.   " Maintenance Service Charge (MSC)

          cv_maintenance_service_charge = cv_maintenance_service_charge + <fs_dberchz>-nettobtr.

        WHEN co_zdissc. " Senior Citizen Discount

          cv_senior_citizen_dscnt = cv_senior_citizen_dscnt + <fs_dberchz>-nettobtr.

        WHEN co_zotr.   " 2012 One Time Tariff Reduction

          cv_tariff_reduction = cv_tariff_reduction + <fs_dberchz>-nettobtr.

        WHEN co_zstm.   " Special Transitory Mechanism

          cv_transitory_mechanism = cv_transitory_mechanism + <fs_dberchz>-nettobtr.

        WHEN co_zpia.   " Payment Incentive Adjustment

          cv_payment_incentive_adjustmnt = cv_payment_incentive_adjustmnt + <fs_dberchz>-nettobtr.
...
ENDFORM.                    " GET_BILLING_DETAILS_ARCHIVED
*&---------------------------------------------------------------------*
*&      Form  GET_ARCHIVED_CHARGES
*&---------------------------------------------------------------------*
*       Extract charges from archive
*----------------------------------------------------------------------*
*      -->P_IV_BELNR    Billing document number
*      -->P_LT_DBERCHZ  Charges
*----------------------------------------------------------------------*
FORM get_archived_charges  TABLES   pt_dberchz LIKE gt_dberchz
                           USING    pv_belnr.
  TYPES : BEGIN OF ty_selection,
            belnr    TYPE dberchz3-belnr,
          END OF ty_selection.
  ...
  IF sy-subrc EQ 0.
    ...
    IF sy-subrc EQ 0.
      ...
      IF sy-subrc EQ 0.
        ...
        LOOP AT lt_dberchz1 ASSIGNING <fs_dberchz1>.
          LOOP AT lt_dberchz3 ASSIGNING <fs_dberchz3> WHERE belzeile EQ <fs_dberchz1>-belzeile.
            APPEND INITIAL LINE TO pt_dberchz ASSIGNING <fs_dberchz>.
            <fs_dberchz>-belnr    = <fs_dberchz1>-belnr.
            <fs_dberchz>-belzeile = <fs_dberchz1>-belzeile.
            <fs_dberchz>-belzart  = <fs_dberchz1>-belzart.
            <fs_dberchz>-nettobtr = <fs_dberchz3>-nettobtr.
          ENDLOOP.
        ENDLOOP.
        UNASSIGN <fs_dberchz1>.
        UNASSIGN <fs_dberchz3>.
    ...
    ENDIF.
  ENDIF.
ENDFORM.                    " GET_ARCHIVED_CHARGES
*&---------------------------------------------------------------------*
*&      Form  GET_REFNO_DUEDATE_ARCHIVE
*&---------------------------------------------------------------------*
*       Extract reference number and due date from Archive
*----------------------------------------------------------------------*
*      -->PV_VKONT      Contract account number
*      -->PV_FROM_DATE  From date
*      -->PV_TO_DATE    To date
*----------------------------------------------------------------------*
FORM get_refno_duedate_archive  TABLES    pt_refno       LIKE gt_refno
                                 USING    pv_vkont       TYPE dfkkop-vkont
                                          pv_from_date   TYPE dfkkop-budat
                                          pv_to_date     TYPE dfkkop-budat.
*  TYPES : BEGIN OF ty_selection,
...
*          END OF ty_selection.
  ...
  DATA:   "lv_selection     TYPE ty_selection,
          ...
          lv_doc_data      TYPE as_t_tablebuffer,
          lv_doc_data2     TYPE astablebuffer,
          lv_dfkkop        TYPE dfkkop.

  DATA:   "lt_obligatory_fields  TYPE STANDARD TABLE OF fieldname,
          lt_fieldcat           TYPE STANDARD TABLE OF aind_str1-archindex,
          lt_result             TYPE STANDARD TABLE OF aind_arkey,
          lt_prueflos           TYPE STANDARD TABLE OF aind_arkey,
          lt_dfkkop             TYPE STANDARD TABLE OF dfkkop.

  DATA:   lt_selections TYPE rsds_frange_t,
          lt_vkont      TYPE TABLE OF rsdsselopt,
          lt_cpudt      TYPE TABLE OF rsdsselopt.
  ...
  "-- Input value for CPUDT
  APPEND INITIAL LINE TO lt_cpudt ASSIGNING <fs_selopt>.
  <fs_selopt>-sign   = 'I'.
  <fs_selopt>-option = 'BT'.
  <fs_selopt>-low    = pv_from_date.
  <fs_selopt>-high   = pv_to_date.
  ...
  APPEND INITIAL LINE TO lt_selections ASSIGNING <fs_selections>.
  <fs_selections>-fieldname = 'CPUDT'.
  LOOP AT lt_cpudt ASSIGNING <fs_any>.
    APPEND INITIAL LINE TO <fs_selections>-selopt_t ASSIGNING <fs_selopt>.
    MOVE-CORRESPONDING <fs_any> to <fs_selopt>.
  ENDLOOP.
  ...
  "APPEND 'VKONT'    TO lt_obligatory_fields.

  SELECT archindex
  INTO TABLE lt_fieldcat
  FROM   aind_str1
  WHERE  object EQ 'FI_MKKDOC'
  AND    itype  EQ 'S'.
  ...
  LOOP AT lt_fieldcat INTO lv_fieldcat.
    CALL FUNCTION 'AS_API_READ'
      EXPORTING
        i_fieldcat                = lv_fieldcat
        i_selections              = lt_selections[] "lv_selection
*        i_obligatory_fields       = lt_obligatory_fields[]
*       i_selections              = lt_range
*       i_maxrows                 = 1
      IMPORTING
  ...
  IF sy-subrc EQ 0.
    ...
    "-- DFKKOP Table
    READ TABLE lv_doc_data WITH KEY tabname = 'DFKKOP' INTO lv_doc_data2.
    IF sy-subrc EQ 0.
      ASSIGN lv_doc_data2-tabref->* TO <fs_doc_data>.
      lt_dfkkop = <fs_doc_data>.

      LOOP AT lt_dfkkop ASSIGNING <fs_dfkkop> WHERE budat BETWEEN pv_from_date AND pv_to_date.
        APPEND INITIAL LINE TO pt_refno ASSIGNING <fs_refno>.
        <fs_refno>-refno    = <fs_dfkkop>-xblnr.
        <fs_refno>-due_date = <fs_dfkkop>-faedn.
      ENDLOOP.
      UNASSIGN <fs_dfkkop>.
    ENDIF.
  ENDIF.
ENDFORM.                    " GET_REFNO_DUEDATE_ARCHIVE

*&---------------------------------------------------------------------*
*&      Form  GET_CONSUMPTION_ARCHIVE
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*      -->IV_BILLING_PERIOD  text
*      <--CV_CONSUMPTION1    text
*----------------------------------------------------------------------*
FORM get_consumption_archive  USING    iv_belnr
                                       iv_adatsoll
                                       iv_installation
                              CHANGING cv_consumption.
  ...
  ELSE.
    "--Get consumption from Archive
    PERFORM get_archived_consumption    USING iv_belnr
                                        CHANGING lv_amount.
  ENDIF.

  cv_consumption = lv_amount.
  CONDENSE cv_consumption.
  ...
  IF gv_archived ne 'X'.
    SELECT SUM( dberchz3~nettobtr )
      ...
      ON  dberchz3~belnr    = dberchz1~belnr
      AND dberchz3~belzeile = dberchz1~belzeile
      WHERE dberchz3~belnr  = iv_belnr
        AND dberchz3~mwskz <> space
        AND dberchz3~preistuf = space
        AND preis LIKE 'CERA%'.
  ELSE.
    "--Get CERA from Archive
    PERFORM get_archived_cera  USING iv_belnr
                               CHANGING lv_cera.
  ENDIF.
  ...
  IF lv_cera > 0.
    " Remove the decimal points( decimals is always zero )
    lv_temp = lv_cera.
    cv_consumption = lv_temp.
  ENDIF.

  SELECT SINGLE ablbelnr
    FROM  eablg
    INTO  lv_ablbelnr
    WHERE anlage   = iv_installation AND
          adatsoll = iv_adatsoll.
  IF sy-subrc = 0.
    SELECT SINGLE istablart
      FROM dberchv
      INTO lv_istablart
      WHERE belnr = iv_belnr AND
            ablbelnr = lv_ablbelnr.
  ENDIF.

  CONDENSE cv_consumption.
  IF NOT cv_consumption IS INITIAL.
    CASE lv_istablart.
      WHEN 93.
        CONCATENATE cv_consumption
                    co_adj
                    INTO cv_consumption
                    SEPARATED BY space.
      WHEN 91.
...
ENDFORM.                    " GET_CONSUMPTION_ARCHIVE