...
*"----------------------------------------------------------------------
  ...
  DATA: lv_objectkey                  TYPE swr_struct-object_key,
        ...
        "-- GDIMALIWAT 02/19/2014
        lv_timein_check               TYPE uzeit,
        lv_timeout_check              TYPE uzeit,
        lv_starttime_check            TYPE uzeit,
        lv_endtime_check              TYPE uzeit.
  "-- End of Modifs by GDIMALIWAT 02/19/2014
...
*         Check if holiday
* Start - DV2K914607 - PMEDIOS - HR.TM.OT Application.v15
*        IF ls_t001p IS NOT INITIAL.                                     " + CDELAPENA 02/05/2014
* End - DV2K914607 - PMEDIOS - HR.TM.OT Application.v15
...
*-- Start Add CDELAPENA 02/05/2014
        IF ls_t550a-sollz = 0.
          lv_nonworking = co_x.
        ENDIF.
        "-- Added by GDIMALIWAT 02/19/2014
        PERFORM f_convert_time USING ls_ot-otstart_time
                               CHANGING lv_starttime_check.
        PERFORM f_convert_time USING ls_ot-otend_time
                               CHANGING lv_endtime_check.
        PERFORM f_convert_time USING ls_ot-timein
                               CHANGING lv_timein_check.
        PERFORM f_convert_time USING ls_ot-timeout
                               CHANGING lv_timeout_check.

        IF lv_nonworking IS INITIAL.
          IF ls_ot-start_date NE ls_ot-end_date.
            IF lv_starttime_check GT lv_endtime_check.
              IF ls_t550a-sobeg LT ls_t550a-soend.
                IF lv_starttime_check GE ls_t550a-sobeg AND lv_starttime_check LE ls_t550a-soend.
                  "--Start Time within working Period
                ELSE.
                  IF lv_endtime_check GE ls_t550a-sobeg AND lv_endtime_check LE ls_t550a-soend.
                    "--Start Time within working Period
                  ELSE.
                    lv_nonworking = co_x.
                  ENDIF.
                ENDIF.
              ELSE.
                IF lv_starttime_check LE ls_t550a-sobeg AND lv_starttime_check GE ls_t550a-soend.
                  "--Start Time within working Period
                ELSE.
                  IF lv_endtime_check LE ls_t550a-sobeg AND lv_endtime_check GE ls_t550a-soend.
                    "--Start Time within working Period
                  ELSE.
                    lv_nonworking = co_x.
                  ENDIF.
            ...
            ENDIF.
          ELSE.
            IF lv_endtime_check GT lv_starttime_check.
              IF lv_starttime_check GE ls_t550a-sobeg AND lv_starttime_check LE ls_t550a-soend.
                "--Start Time within working Period
              ELSE.
                IF lv_endtime_check GE ls_t550a-sobeg AND lv_endtime_check LE ls_t550a-soend.
...
*               IF ls_day-freeday EQ co_x OR ls_day-holiday EQ co_x.        " - CDELAPENA 02/05/2014
      ...
      ELSE.                      " IF ls_ot-start_date EQ ls_ot-end_date.
*        check if holiday
        CLEAR: lv_nonworking,
               ls_day.
        CALL FUNCTION 'DAY_ATTRIBUTES_GET'
