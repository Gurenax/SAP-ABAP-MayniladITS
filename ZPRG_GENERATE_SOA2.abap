...
* 01/25/2013 | RANDYSY    | Change Query of MR Scheds                           | TN#26304
* 02/27/2013 | RANDYSY    | Change SOA Format                                   | TN#28322
* 03/25/2013 | ANNACLO    | Attached new SOA Format and modify validtion in     | TN#30334
*            | ANNACLO    | passing of the parameters to the form               |
* 04/22/2013 | GDIMALIWAT | Fixed lowest_date value for tagging of Disc Tag     | TN#33029
* 04/29/2013 | RANDYSY    | Fixed date and time for fetching PUA from FM        | TN#
* 06/04/2013 | GDIMALIWAT | Fixed backtracking of 1st Day for holidays/weekends | TN#35942
* 06/27/2013 | RANDYSY    | MSC and DISSC data change source of data            | TN#36209
* 07/12/2013 | RANDYSY    | Add filter for Current Charges, ABWTP NE 'R'        | TN#39659
* 10/11/2013 | RANDYSY    | Fix Due Date, Instal. Due and PUA                   | TN#41237
* 10/29/2013 | RANDYSY    | Adjusted Layout for Bill.History and Payment        | TN#47542
* 11/06/2013 | RANDYSY    | Fix Clearing Docs for Miscellaneous Charges         | TN#47945
* 01/21/2014 | RANDYSY    | Change address structure                            | TN#54322
* 02/20/2014 | GDIMALIWAT | Fix for PUA / Overpayment not showing in Smartform  | TN#56978
* 02/28/2014 | GDIMALIWAT | Fix for function module ZMWOSB_GET_PUA              | TN#56978
* 03/25/2014 | GDIMALIWAT | Added Start of Bill Period for ZMWOSB_GET_PUA       | TN#59200
* 04/25/2014 | GDIMALIWAT | Added Misc computation in FM ZMWOSB_GET_PUA         | TN#59200
*--------------------------------------------------------------------------------------------------
...
*                                             lv_tar_reduction
        ...
        SELECT SINGLE budat opbel
          INTO (lv_budat, wa_zbilldet-opbel)
        FROM   erchc
        WHERE  belnr EQ <fs_soa>-belnr.

        IF sy-subrc EQ 0.
          wa_zbilldet-budat = lv_budat.
        ENDIF.
...
*        IF lv_datatype EQ 'NUMC'.
        ...
        REPLACE ALL OCCURRENCES OF: 'ADJ' IN wa_zbilldet-consump WITH space,
                                    'AVE' IN wa_zbilldet-consump WITH space.
...
FORM disctag.
  ...
  ELSE.
    ...
    ELSE.
      "-- Check if Due Date of PUA is within Billing Period then Removes PUA
      IF lv_due_date GE <fs_soa>-prevrdgdt AND lv_due_date LE <fs_soa>-presrdgdt.
        CLEAR: <fs_soa>-pua.
      ENDIF.
    ENDIF.
  ENDIF.
...
*--- Comment For Address Cleansing - GDIMALIWAT 02/20/2014
...
*         INTO pv_address
*    SEPARATED BY space.
*--- End -- Comment For Address Cleansing - GDIMALIWAT 02/20/2014

*--- Uncomment For Address Cleansing - GDIMALIWAT 02/20/2014
  DATA: lv_address_number  TYPE ad_addrnum, "-- Address Number
        lv_house_num1      TYPE ad_hsnm1,   "-- House Number
        lv_str_suppl1      TYPE ad_strspp1, "-- Street 2
        lv_house_num2      TYPE ad_hsnm2,   "-- Supplement
        lv_street          TYPE ad_street,  "-- Street
        lv_str_suppl3      TYPE ad_strspp3, "-- Street 4
        lv_location        TYPE ad_lctn,    "-- Street 5
        lv_city2           TYPE ad_city2,   "-- District
        lv_city1           TYPE ad_city1.   "-- City

  "-- Clear Local & Changing Variables
  ...
  IF sy-subrc EQ 0.
    ...
    SELECT SINGLE house_num1
      ...
      INTO (lv_house_num1,
            ...
            lv_city1)
    FROM    adrc
    WHERE   addrnumber EQ lv_address_number.

    IF sy-subrc EQ 0.
      CONDENSE: "lv_address_number,
                lv_house_num1,
                lv_str_suppl1,
                lv_house_num2,
                lv_street,
      ...
      CONCATENATE "lv_address_number
                  ...
                  lv_city2
                  lv_city1
             INTO pv_address SEPARATED BY space.
    ENDIF.
  ENDIF.
*--- End -- Uncomment For Address Cleansing - GDIMALIWAT 02/20/2014
ENDFORM.                    " GET_ADDRESS
...
*      LOOP AT lt_fkkop ASSIGNING <fs_fkkop> WHERE faedn GE lv_start_bill "<fs_soa>-prevrdgdt
*                                              AND faedn LE lv_end_bill   "<fs_soa>-presrdgdt
      LOOP AT lt_fkkop ASSIGNING <fs_fkkop> WHERE augdt GE lv_start_bill
                                              AND augdt LE lv_end_bill
  ...
  AND    augrd NE '05'
*  AND    augst EQ space
  AND    augdt LE tmp_lastdayofmonth
  AND    abwbl EQ space.
...
FORM get_pua_from_fm_soa  USING    pv_vkont    TYPE vkont_kk
  ...
  IF sy-subrc EQ 0.
    ...
    lv_end_time = lv_end_time - lv_less_sec. "-- Commented by GDIMALIWAT 02/21/2014
    "-- Get start of billing period
    lv_start_bill = pv_prevrdgdt + 1.
    lv_end_bill   = pv_presrdgdt.

    "-- Call FM for Getting PUA (Single Billing/Invoicing Reimplementation)
    CALL FUNCTION 'ZMWOSB_GET_PUA'
      EXPORTING
        I_VKONT               = pv_vkont
        I_BASED_LINE_DATE     = lv_based_date
        I_END_TIME            = lv_end_time
        I_START_BILL_PRD      = lv_start_bill
        I_END_BILL_PRD        = lv_end_bill
      IMPORTING
        E_PUA                 = pv_pua
...
FORM get_reading_details  USING    pv_vkont        TYPE vkont_kk
  ...
  DATA: lv_holiday     TYPE sysubrc,
        ...
        lv_firstday    TYPE sydatum,
        lv_sched_bd    TYPE abrdats.
  ...
  IF sy-subrc EQ 0 AND pv_anlage IS NOT INITIAL.
    ...
    IF sy-subrc EQ 0.
      ...
      lv_firstday = pv_firstday.

      WHILE lv_holiday EQ 0 OR lv_weekend EQ 0.
        SUBTRACT 1 FROM lv_firstday.  "pv_firstday.

        "-- Re-check if Holiday
        PERFORM check_holiday   USING    lv_firstday
                                         lv_calendar_id
                                CHANGING lv_holiday.
        "-- Re-check if Weekend
        PERFORM check_weekend   USING    lv_firstday
                                CHANGING lv_weekend.
      ENDWHILE.
    ENDIF.
    ...
    CALL FUNCTION 'BAPI_MTRREADDOC_GETLIST'
      EXPORTING
        ...
        mrdatefrom        = pv_firstday  "pv_firstday
        mrdateto          = pv_lastday
...
*      READ TABLE lt_eablg ASSIGNING <fs_eablg> WITH KEY mrreason = co_mrreason.
      ...
      LOOP AT lt_eablg ASSIGNING <fs_eablg> WHERE mrreason = co_mrreason.
*      IF sy-subrc EQ 0.
        IF <fs_eablg>-targetbillingdate+0(6) EQ lv_sched_bd+0(6).
...
*      IF sy-subrc EQ 0.
      IF lv_consumption2 IS NOT INITIAL.
        ...
        IF lv_datatype NE 'NUMC'.
          REPLACE ALL OCCURRENCES OF: 'ADJ' IN lv_consumption2 WITH space,
                                      'AVE' IN lv_consumption2 WITH space.
        ENDIF.
