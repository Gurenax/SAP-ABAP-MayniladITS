*&-------------------------------------------------------------------------------------------*
* PROGRAM ID      : ZRISU_DUNNING_LIST2
...
* 04/15/2013 | GDIMALIWAT  | 31700            | Program modified to fix protested bill condition
* 04/24/2013 | GDIMALIWAT  | 31700            | Program modified to add miscellaneous column
* 05/30/2013 | GDIMALIWAT  | 31700            | Program modified to change current condition to 0-25 days and PUA condition to 26-60 days
* 05/30/2013 | GDIMALIWAT  | 31700            | Program modified to change e-mail recipents (Air 21)
* 06/06/2013 | GDIMALIWAT  | 31700            | Program modified to fix missing data - Name, MRU, RSG, Device
* 06/10/2013 | GDIMALIWAT  | 31700            | Program modified to fix other unpaid data
*& Changes made in Arangkada Dev ------------------------------------------------------------*
* 07/10/2013 | GDIMALIWAT  | 31700            | Program modified to remove checking of condoned accounts
* 07/10/2013 | GDIMALIWAT  | 31700            | Program modified to add the validation - oldest open bill > 60 days
* 07/10/2013 | GDIMALIWAT  | 31700            | Program modified to remove HVORG 0100 and 0200 in main extraction condition
* 07/12/2013 | RANDYSY     | 31700            | Added filter for Reversed Posting - for Due Balance
* 07/15/2013 | GDIMALIWAT  | 31700            | Removed -7 days computation from lv_premise_param
* 09/06/2013 | GDIMALIWAT  | 31700            | Added condition lv_misc >= 0 in Dunning sub-routine
...
*& Changes made in Standard Dev after SAP upgrade -------------------------------------------*
* 10/07/2013 | GDIMALIWAT  | 31700            | Re-created dunning list program in Standard Dev after SAP upgrade
*&-------------------------------------------------------------------------------------------*

REPORT zrisu_dunning_list2 NO STANDARD PAGE HEADING LINE-SIZE 280.
...
*  ELSE
  ...
  DESCRIBE TABLE gt_dunn2 LINES lv_count_dunn2.
...
*      ELSE.
      ...
      IF lv_overdue > 80 AND lv_total > 0 AND lv_misc >= 0.
        APPEND dunnlist3.
      ENDIF.
    ENDIF.
    "    ENDIF.
...
*                                        CHANGING pv_current.
  ...
  lv_days25 = gv_highest_duedate - 25.
  ...
  AND budat NE '00000000'
  AND abwbl EQ space.
  "AND xragl NE 'X'. "-- Added by RANDYSY 07/12/2013 - TN#31700
  ...
  IF sy-subrc EQ 0.
    LOOP AT lt_misc_inst ASSIGNING <fs_misc_inst>.
      ...
      IF sy-subrc EQ 0.
        ...
        LOOP AT lt_sfkkop ASSIGNING <fs_sfkkop> WHERE faedn GT gv_highest_duedate.
...
FORM append_main_itab2  USING   pv_gsber    pv_rsg      pv_ableinh    pv_gpart    pv_vkont    pv_printdoc
                                ...
                                pv_misc pv_total    pv_sortfld  pv_device     pv_nodays   pv_oc       pv_oc2.
