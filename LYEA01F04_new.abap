...
** No outsorting in case of pre-billing     "commented by GDIMALIWAT 10/08/2013
*  CHECK p_obj-prebill-active IS INITIAL.   "commented by GDIMALIWAT 10/08/2013
...
* >>>>> note 174386
        s_subrc = sy-subrc.
        mac_msg_log_on.
        IF s_subrc > 1.                           "note 982412
          mac_msg_last_repeat co_msg_error general_fault.
* <<<<< note 1747386
*       >>>>> note982412
        ELSEIF s_subrc = 1.                                 "n1747386
...
** >>>>> note 1656939                                                           "Start of comment by GDIMALIWAT 10/08/2013
*        IF s_te418-adatsoll < wte417-abrdats OR
*           s_te418-adatsoll = xy_obj-abr-endabrpe.          "n1776131
** determine next schedule date because otherwise incorrect
** advance billing timeslice (from greater than to)
*          datum = s_te418-adatsoll.
*          CLEAR s_te418.
*          CALL FUNCTION 'ISU_S_BIPR_MRR_DETERMINE'
*            EXPORTING
*              x_portion     = wate422-portion
...
*          ENDIF.
*        ENDIF.
** <<<<< note 1656939                                                             "End of comment by GDIMALIWAT 10/08/2013
...
*        temp_begana_estb                                                   "Commented by GDIMALIWAT
*                      TYPE   endabrpe.                      "n1468381      "Commented by GDIMALIWAT
...
*   ALle Ableseergebnisse zur Korrekturperiode einlesen
    IF xy_obj-fl-estb_mode IS INITIAL.
      ...
      ENDLOOP.                           " xy_izwst
*      IF sy-subrc <> 0.                                                          "Start of comment by GDIMALIWAT 10/08/2013
** >>>>> note 1685758
**   for single past time slices the izwst_back is not needed to create
**   ithg_back entries
*        READ TABLE xy_obj-iperiod TRANSPORTING NO FIELDS
*        WITH KEY pbascat = co_past_billdoc.
*        IF sy-subrc <> 0.
*          CHECK NOT izwst_back IS INITIAL.                  "n1466330
*        ENDIF.
*      ENDIF.
** <<<<< note 1685758                                                              "End of comment by GDIMALIWAT 10/08/2013
    ELSE.
      ...
      WHERE pbascat = co_past_billdoc.
        ...
        EXIT.
      ENDLOOP.
** if no izwst_back is found, no ithg_back can be created                          "Start of comment by GDIMALIWAT 10/08/2013
** furthermore the biling can be abborted at gas procedure change
*      CHECK NOT izwst_back IS INITIAL.                      "n1658324             "End of comment by GDIMALIWAT 10/08/2013
    ENDIF.
*   -----------------------------------------------------
*   Zu jeder Korrekturperiode lokale IZW erstellen
    CALL FUNCTION 'ISU_IZW_ADJUST'
...
*         Gas billing for past and actual period in one timeslice
          REFRESH izw_back.
** >>>>> note 1468381                                                   "Start of comment by GDIMALIWAT 10/08/2013
*          temp_begana_estb = xy_obj-date-begana_estb.
*          IF xy_obj-date-begana_estb > xy_obj-date-begana.
*            xy_obj-date-begana_estb = xy_obj-date-begana.
*          ENDIF.
** <<<<< note 1468381                                                    "End of comment by GDIMALIWAT 10/08/2013
          CALL FUNCTION 'ISU_IZW_ADJUST'
            EXPORTING
              x_obj         = xy_obj
            CHANGING
          ...
          IF sy-subrc <> 0.
            mac_err_repeat general_fault.
          ENDIF.
*          xy_obj-date-begana_estb = temp_begana_estb.       "n 1468381     "Commented by GDIMALIWAT 10/08/2013
...
** advance billing period is adatsoll + 1 -> in case of the interim bill      "Start of comment by GDIMALIWAT 10/08/2013
** ends on a day where a TE418 exists error AJ-451 occurs (from-date is
** one day after to-date)
*  sav_adatsoll = xy_obj-trig-etrg-adatsoll + 1.             "n1443619        "End of comment by GDIMALIWAT 10/08/2013
*>>>> Note 876320
  CALL FUNCTION 'ISU_DB_TE418_SLCT_N_ADATSOLL_T'
    EXPORTING
      x_termschl    = xy_obj-schedule-ableinh
*      x_datum       = sav_adatsoll                          "n1443619       "Commented by GDIMALIWAT 10/08/2013
      x_datum       = XY_OBJ-TRIG-ETRG-ADATSOLL                              "Added by GDIMALIWAT 10/08/2013
    IMPORTING
      y_te418       = y_te418
    EXCEPTIONS
      not_found     = 1
...
** >>>>> note 1369808                                                         "Start of comment by GDIMALIWAT 10/08/2013
...
*    PERFORM get_special_begin_for_dpc_est CHANGING begana
*                                                   xy_obj-pbill.
** <<<<< note  1369808                                                        "End of comment by GDIMALIWAT 10/08/2013
