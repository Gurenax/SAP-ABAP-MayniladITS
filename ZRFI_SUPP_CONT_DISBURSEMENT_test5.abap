...
DATA: gt_bsak          TYPE TABLE OF ty_bsak,        "All Payment Docs
      ...
      gt_bsak2         TYPE TABLE OF ty_bsak2,       "Scenario 2
...
FORM begin_extraction .
  ...
  DATA: lv_hkont type bsis-hkont,
        ...
        lv_buzid type bsis-buzid,
        lv_sgtxt type bsis-sgtxt.

  IF so_blart IS NOT INITIAL.
    SELECT bukrs belnr gjahr rebzg rebzj lifnr empfb zlsch waers budat dmbtr wrbtr "All Payment Docs
    INTO TABLE gt_bsak
    FROM bsak
    ...
    LOOP AT gt_bsak ASSIGNING <fs_bsak>.
      ...
      IF sy-subrc EQ 0.
        LOOP AT gt_bsak_amount ASSIGNING <fs_bsak_amount>.
          ...
          ADD <fs_bsak_amount>-wrbtr TO gv_netpay_wrbtr_amt.       "Net payment amount
        ENDLOOP.
...
*        Start of Scenario 1
      ...
      ELSE.
*        Start of Scenario 2
        CLEAR: gv_netpay_dmbtr_amt, gv_netpay_wrbtr_amt.
        REFRESH gt_bsak_amount.

*       Scenario 2 Non Payment Doc
        IF <fs_bsak>-rebzg IS NOT INITIAL AND <fs_bsak>-rebzj IS NOT INITIAL.

          CLEAR: lv_hkont, lv_dmbtr,  lv_wrbtr, lv_shkzg, lv_buzid, lv_sgtxt.

          "Net payment amount / Net payment amount in php
          ADD <fs_bsak>-dmbtr TO gv_netpay_dmbtr_amt.       "Net payment amount in php
          ADD <fs_bsak>-wrbtr TO gv_netpay_wrbtr_amt.       "Net payment amount

          "Replace the BSAK hkont with a BSIS hkont
          SELECT SINGLE hkont dmbtr wrbtr shkzg buzid sgtxt
          INTO (lv_hkont, lv_dmbtr,  lv_wrbtr, lv_shkzg, lv_buzid, lv_sgtxt)
          FROM bsis
          WHERE bukrs EQ pa_bukrs
            AND belnr EQ <fs_bsak>-rebzg
            AND gjahr EQ <fs_bsak>-rebzj.

          IF lv_hkont EQ '2161000000' OR lv_hkont EQ '1160103000'.
            IF lv_sgtxt CS 'MWSS MOE'.
              APPEND INITIAL LINE TO gt_bsis2 ASSIGNING <fs_bsis2>.
              <fs_bsis2>-hkont = lv_hkont.
              <fs_bsis2>-dmbtr = lv_dmbtr.
              <fs_bsis2>-wrbtr = lv_wrbtr.
              <fs_bsis2>-sgtxt = lv_sgtxt.
              <fs_bsis2>-shkzg = lv_shkzg.
            ENDIF.
          ELSEIF lv_buzid NE 'T'.
            APPEND INITIAL LINE TO gt_bsis2 ASSIGNING <fs_bsis2>.
            <fs_bsis2>-hkont = lv_hkont.
            <fs_bsis2>-dmbtr = lv_dmbtr.
            <fs_bsis2>-wrbtr = lv_wrbtr.
...
*&---------------------------------------------------------------------*
*       Scenario 1
*       Non-payment documents which are cleared by the payment document
*----------------------------------------------------------------------*
form SCENARIO1 .
  ...
  LOOP AT gt_bsas ASSIGNING <fs_bsas>.
    ...
    SELECT SINGLE hkont dmbtr wrbtr shkzg buzid sgtxt
    INTO (<fs_bsas>-hkont, <fs_bsas>-dmbtr, <fs_bsas>-wrbtr, <fs_bsas>-shkzg, <fs_bsas>-buzid, <fs_bsas>-sgtxt)
    FROM bsis
    WHERE bukrs EQ pa_bukrs
...
*&---------------------------------------------------------------------*
*       Scenario 2
*       Non-payment documents tagged as invoice reference doc. no. and FY of the payment document (if any)
*----------------------------------------------------------------------*
form SCENARIO2 .
...
*&---------------------------------------------------------------------*
*       Scenario 3
*       There are no non-payment documents from the scenario 1 or 2
*----------------------------------------------------------------------*
form SCENARIO3 .
