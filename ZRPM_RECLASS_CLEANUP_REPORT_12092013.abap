...
*&
DATA: gt_bsis1    TYPE TABLE OF ty_bsis,                   "#EC NEEDED
      gt_bsis2    TYPE TABLE OF ty_bsis,                   "#EC NEEDED

      gt_mseg     TYPE TABLE OF ty_mseg,                   "#EC NEEDED
      gt_mseg2    TYPE TABLE OF ty_mseg,                   "#EC NEEDED
...
*&
FIELD-SYMBOLS:  <fs_bsis>        TYPE ty_bsis,            "#EC NEEDED
                <fs_bsis2>       TYPE ty_bsis2,           "#EC NEEDED
                <fs_mseg>        TYPE ty_mseg,            "#EC NEEDED
...
FORM display_report.
  ...
  mc_change_column_names: 'BLART'     text-004 text-004 text-019,
                          'BLDAT'     text-005 text-005 text-020,
                          'BUDAT'     text-006 text-006 text-021,
                          'BELNR'     text-007 text-007 text-022,
                          'WBS'       text-008 text-008 text-023,
                          'AUFNR'     text-009 text-009 text-024,
                          'DMBTR'     text-010 text-010 text-025,
                          'WERKS'     text-011 text-011 text-026,
                          'INGPR'     text-012 text-012 text-027,
                          'ILART'     text-013 text-013 text-028,
                          'VAPLZ'     text-014 text-014 text-029,
                          'MBLNR'     text-015 text-015 text-030,
                          'ZEILE'     text-016 text-016 text-031,
                          'MATNR'     text-017 text-017 text-032,
                          'MENGE'     text-018 text-018 text-033.
...
FORM query_by_wbs.
  ...
  REFRESH gt_mseg2.
  "MSEG
  SELECT mblnr matnr menge aufnr mat_pspnr werks sakto nplnr zeile dmbtr
  INTO TABLE gt_mseg2
  FROM mseg
...
*      lv_measure = lv_wbs(lv_wbs_len).
      ...
      "BSIS XBLNR
      SELECT belnr dmbtr shkzg blart bldat budat buzei xblnr
      INTO TABLE gt_bsis2
      FROM bsis
      ...
      IF sy-subrc EQ 0.
        LOOP AT gt_bsis2 ASSIGNING <fs_bsis>.
          ...
          SHIFT <fs_output>-aufnr LEFT DELETING LEADING '0'.
          IF <fs_bsis>-shkzg EQ 'H'.
            <fs_output>-dmbtr   = <fs_bsis>-dmbtr * -1.
            <fs_output>-menge   = <fs_mseg>-menge * -1.
          ELSE.
          ...
          <fs_output>-zeile     = <fs_mseg>-zeile.
          <fs_output>-matnr     = <fs_mseg>-matnr.
          SHIFT <fs_output>-matnr LEFT DELETING LEADING '0'.
  ...
  LOOP AT lt_bsis ASSIGNING <fs_bsis2>.
    ...
    INTO lv_awkey
    FROM bkpf
    WHERE belnr EQ <fs_bsis2>-belnr. "#EC WARNOK "#EC CI_NOFIRST
    ...
    IF sy-subrc EQ 0.
      ...
      WHERE mblnr EQ lv_mblnr
        AND zeile EQ lv_zeile. "#EC WARNOK
    ENDIF.
    ...
    <fs_output>-wbs       = lv_wbs.
    <fs_output>-aufnr     = lr_mseg-aufnr.
    SHIFT <fs_output>-aufnr LEFT DELETING LEADING '0'.
    IF <fs_bsis2>-shkzg EQ 'H'.
      <fs_output>-dmbtr   = <fs_bsis2>-dmbtr * -1.
      <fs_output>-menge   = lr_mseg-menge * -1.
    ELSE.
      <fs_output>-dmbtr   = <fs_bsis2>-dmbtr.
      <fs_output>-menge   = lr_mseg-menge.
    ENDIF.
    <fs_output>-werks     = <fs_bsis2>-werks.
    <fs_output>-ingpr     = ''.
    <fs_output>-ilart     = ''.
    <fs_output>-vaplz     = ''.
    <fs_output>-mblnr     = lr_mseg-mblnr.
    <fs_output>-zeile     = lr_mseg-zeile.
    <fs_output>-matnr     = lr_mseg-matnr.
    SHIFT <fs_output>-matnr LEFT DELETING LEADING '0'.
