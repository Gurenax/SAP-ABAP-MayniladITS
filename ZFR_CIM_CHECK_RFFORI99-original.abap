...
** START OF INSERTION BY CIM 09/26/2001
  Select * from bsak where bukrs = regup-zbukr and
                           lifnr = regup-lifnr and
                           augbl = regup-vblnr and
                           sgtxt ne ' '.
     regup-sgtxt = bsak-sgtxt.
  endselect.
** END OF INSERTION CIM 09/26/2001
  ...
  ON CHANGE OF reguh-zbukr OR reguh-rzawe.
    SELECT SINGLE * FROM t042e
      WHERE zbukr EQ reguh-zbukr
        AND zlsch EQ reguh-rzawe.
    IF hlp_t021m_h-srvar NE t042e-svarh.
      ...
      ENDIF.
    ENDIF.
    IF hlp_t021m_p-srvar NE t042e-svarp.
...
* fill net total fields with protective asterisks ----------------------
  ...
  TRANSLATE:
    regud-snets USING ' *',
    regud-swnes USING ' *'.
  IF REGUD-SWNES(1) EQ '*'.         " CIM 09/24/2001
    REGUD-SWNES(1) = ' '.           " CIM 09/24/2001
  ENDIF.                            " CIM 09/24/2001
